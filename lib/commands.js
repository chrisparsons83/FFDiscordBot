  ...
  new Promise((resolve, reject) => {
    ...
    if (position === 'dst') {
      ...
      players.forEach((player) => {
        ...
        if (!matched) {
          reject("Player(s) doesn't exist in the database. Please use the player's full name.");
        }
      });
    } else {
      players = args
        ...
        .then((object) => {
          ...
          if (response.includes(',') || response.includes(' and ')) {
            message = `Boris says: **${response}** are in the same tier level at **${position.toUpperCase()}** with ${
              format === 'full' ? '**1 PPR**' : format === 'half' ? '**0.5 PPR**' : '**Standard**'
...
commands['!prediction'] = discordObject =>
  new Promise((resolve, reject) => {
    //reject("Stop trying to make !prediction happen. It's not going to happen.");

    // get prediction
    // reject if theres no prediction
    if (discordObject.args === undefined) {
      reject('You\'d have to give me a prediction!');
    } else {
      // get predictiong string
      const predictionString = discordObject.args;
      // get user that sent the prediction string
      const predictionUser = discordObject.user.username
      //get an random eightball answer
      utilities.eightBall().then(eightBallResponse => {
        //init twit
        const Twitter = new twit(config.twitterTokens);
        //post prediction on twitter
        Twitter.post('statuses/update', {status: predictionString})
        //error catching
        .catch((err) => {
          // reject with error message, probably a better way of handling this instead of everybody in the server seeing the error message posted by the bot
          reject(`Error ${err.statusCode}: ${err.message}`)
        }).then(()=> {
          // respond back with a promise object with the channel name and user name
          resolve({
            channelName: 'predictions',
            message: `${predictionUser}: ${predictionString}`,
            eightball: eightBallResponse
          });
        });
      });
    }
