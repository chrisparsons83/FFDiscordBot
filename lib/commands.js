...
const tubaStats = require('./tubastats.js');
...
commands['%tubastats'] = teamStatsObject => new Promise((resolve, reject) => {
  ...
  if (user_arguments_list.length == 2) {
    target_team = arg[0];
    target_year = arg[1];
  } else if ((arg.length == 2 && arg[1] != '') || arg.length == 1) {
    target_team = arg[0];
  ...
  const valid_team = teamLookup.filter((obj) => {
    if (obj.abbreviation.toLowerCase() === input || obj.mascot.toLowerCase() === input) {
      return obj
    }
  });
  ...
  if (target_team === '') {
  ...
  } else if (Object.keys(tubaStatsData).keys().indexOf(target_year) < 0) {
    reject('Sorry, I only have data for the following years  [- ' + Object.keys(tubaStatsData).keys().join(' - ') + ' -]');
  } else {
    ...
    if (Object.keys(target_year_data).indexOf(teamname) > -1) {
      const statnode = TubaStatNode(target_year_data[teamname]);
      let average_data = null;
      if (target_year_data.indexOf("Average") > -1) {
        average_data = TubaStatNode(target_year_data["Average"]);
      }


      total_plays_str = average_data ? '· (League Avg: ' + total_plays_str + ')' : '';
      average_rush_str = average_data ? '· ' + (statnode.rushPercent - average_data.rushPercent) + '%' : '';
      average_pass_str = average_data ? '· ' + (statnode.passPercent - average_data.passPercent) + '%' : '';
      completions_pct = statnode.completions / statnode.passAttempts
      average_completions_str = average_data ? '· ' + (completions_pct - (average_data.completions - average_data.passAttempts)) + '%' : '';
      average_sack_str = average_data ? '· ' + (statnode.sackPercent - average_data.sackPercent) + '%' : '';

      let sorted_targets = statnode.getSortedTargets()
      let sorted_receptions = statnode.getSortedReceptions()
      ...
      */
      ...
      sorted_targets.forEach((item, index) => {
        target_str += ">" + item.position.toUpperCase() + ": " + item.targetPct + "%  ("+ item.targets+")\n";
      });
      receptions_str = ""
      sorted_receptions.forEach((item, index) => {
        receptions_str += ">" + item.position.toUpperCase() + ": " + item.receptions + " receptions\n";
      });

      embed_fields = [{
          name: `Total Plays: ${statnode.totalPlays} ${total_plays_str}`,
          value: `
          > Rush: ${statnode.rushAttempts} att < ${statnode.rushPercent}% ${average_rush_str} >  \n
          > Pass: ${statnode.passAttempts} att < ${statnode.passPercent}% ${average_pass_str} >  \n
          >> Completions: ${statnode.completions} < ${completions_pct}% ${average_completions_str} >  \n
          >> Sacks: ${statnode.sacks} < ${statnode.sackPercent}% ${average_sack_str} >
        `,
          inline: false
        },
        {
    ...
    const embed = {
      embed: {
        title: `${teamname}`,
        description: `${target_year} Offensive stats`,
        color: 11913417,
        thumbnail: {
          url: teamlogo,
        },
