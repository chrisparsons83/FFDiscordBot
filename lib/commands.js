...
commands['!teamstats'] = function(input) {
  return new Promise( (resolve, reject) => {
    ...
    if (input === '!teamstats') {
      ...
      let embed = stats.getBreakdown(teamname, symbol);
      resolve(embed)
    }
  });
}
...
commands['!depthchart'] = function(string) {
  ...
  return new Promise((resolve, reject) => {
    if (arg.length < 2) {
      ...
      let validPosition = ['qb', 'rb', 'fb', 'wr', 'te', 'kr', 'pr', 'k', 'lt', 'lg', 'c', 'rg', 'rt', 'ldt', 'lde', 'rdt', 
      ...
      if (!(teamname in teamLookup)) {
        ...
        depthChart.getRoster(teamname, position).then(embed => {
          resolve(embed);
        }).catch(err => {
          resolve(err); 
        });
