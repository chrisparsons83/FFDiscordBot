...
commands['!choose'] = function(args) {
  return new Promise( (resolve, reject) => {
    if (args === '!choose') {
      resolve('I need things to choose from!')
    } else {
...
//get team stats
commands['!teamstats'] = function(input) {
  return new Promise( (resolve, reject) => {
    let valid = input.toUpperCase() in teamLookup;
    if (input === '!teamstats') {
      resolve(`I need a team symbol.`);
    } else if (!valid) {
      resolve(`Invalid team symbol.`)
    } else {
      let teamname = teamLookup[input.toUpperCase()].name;
      let symbol = input.toUpperCase();
      let object = stats.getBreakdown(teamname, symbol);
      let embed = {
        "embed": {
          "title": `${teamname}`,
...
commands['!teamtargets'] = function(teamname) {
  return new Promise((resolve, reject) => {
    ...
    if (teamname === '!top8targets') {
      ...
      let team = teamLookup[teamname.toUpperCase()].name;
...
commands['!depthchart'] = function(string) {
  ...
  return new Promise((resolve, reject) => {
    if (arg.length < 2) {
      ...
      let validPosition = ['qb', 'rb', 'fb', 'wr', 'te', 'kr', 'pr', 'k', 'lt', 'lg', 'c', 'rg', 'rt', 'ldt', 'lde', 'ndt', 'rdt', 
      ...
      if (!(teamname in teamLookup)) {
        ...
        depthChart.getRoster(teamname, position)
        .then(object => {
          let embed = {
            "embed": {
          ...
          for (let i = 0; i < object.roster[position].length; i++) {
...
commands['!boris'] = function(string) {
  ...
  return new Promise((resolve, reject) => {
    let validPosition = ['qb', 'rb', 'te', 'wr', 'k', 'dst'];
    let validFormat = ['standard', 'half', 'full'];
    if (args.length < 2) {
      ...
      resolve('Invalid position/format. Valid positions are *qb, rb, wr, te, k , dst*.\nValid format are *standard, half, full*');
    } else {
      let position = args[0].toLowerCase();
      let scoring = args[1].toLowerCase();
      boris.getTier(position, scoring).then(object => {
        let title = `Boris Chen ${object.position} tiers`;
        let url = object.url;
        let embed = {
          "embed": {
...
commands['!find'] = function(string) {
  return new Promise((resolve, reject) => {
    let results = utilities.findPlayer(string);
    if(!results) {
      resolve('Cannot find player');
    } else {
