  ...
  try {
    ...
    if (target_team === '') {
      ...
      if (Object.keys(target_year_data).indexOf(teamname) > -1) {
        ...
        let total_plays_str = average_data ? '· (League Avg: ' + average_data.total_plays + ')' : '';
        let average_rush_str = average_data ? '· ' + (statnode.rushPercent - average_data.rushPercent) + '%' : '';
        let average_pass_str = average_data ? '· ' + (statnode.passPercent - average_data.passPercent) + '%' : '';
        let completions_pct = statnode.completions / statnode.passAttempts
        let average_completions_str = average_data ? '· ' + (completions_pct - (average_data.completions - average_data.passAttempts)) + '%' : '';
        let average_sack_str = average_data ? '· ' + (statnode.sackPercent - average_data.sackPercent) + '%' : '';
