...
commands['!teamtargets'] = object =>
  new Promise((resolve, reject) => {
    const args = object.args.split(/[\s,]/).filter(n => /\S/.test(n));
...
commands['!depthchart'] = (depthChartObject) => {
  const arg = depthChartObject.args.split(/[\s,]/).filter(n => /\S/.test(n));
...
commands['!boris'] = (borisObject) => {
  ...
  let args = string.toLowerCase().split(/[\s,]/).filter(n => /\S/.test(n))
  return new Promise((resolve, reject) => {
...
commands['!snaps'] = discordObject =>
  new Promise((resolve, reject) => {
    if (discordObject.args === undefined) {
      ...
      const args = string.toUpperCase().split(/[\s,]/).filter(n => /\S/.test(n));
      ...
      const validYear = ['2012','2013','2014','2015', '2016', '2017', '2018']
