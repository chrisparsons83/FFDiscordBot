  ...
  if (name === '') {
    ...
    nameObject.message.react('⌛').then(()=>{
      rotoworld.getPlayer(name).then((playerObject) => {
        ...
        let my_react = nameObject.message.reactions.filter((reaction) => {
          if (reaction.me == true && reaction.emoji.name == '⌛') {
            return reaction
          }
        });
        console.log(':::  ' + JSON.stringify(my_react))
        my_react[0].remove(nameObject.botuser).then(function () {
