      ...
      if (Object.keys(target_year_data).indexOf(teamname) > -1) {
        ...
        */
        ...
        sorted_targets.forEach((item, index) => {
          average_position_node = average_data[item.position + 'TargetPct']
          target_str += "- " + item.position.toUpperCase() + ": " + item.targetPct + "%  / " + item.targets + " targets  ( " + average_position_node + "%  / " + average_data.targets + " )\n";
        });
        let receptions_str = ""
        sorted_receptions.forEach((item, index) => {
          average_reception_node = average_data[item.position + 'TargetPct']
...
--- Completions: ${statnode.completions} att / ${completions_pct}%  ( ${average_data.completions} / ${avg_completions_pct}% )
--- Sacks: ${statnode.sacks}  (${sackPercent}%)  ( ${average_data.sacks} / ${sackPercent}% )
            `,
            inline: false
          },
          {
