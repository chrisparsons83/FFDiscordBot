...
const drivestats = require("./drivestats.js")
  ...
  new Promise((resolve, reject) => {
    ...
    lib4for4
      ...
      .then((resultObject) => {
        ...
        } (${utilities.convertToPickPosition(resultObject.ADP.ESPN)})`;
        returnString += `, 4f4: ${
          resultObject.ADP.fourF4
        } (${utilities.convertToPickPosition(resultObject.ADP.fourF4)})`;
        returnString += `, Yahoo: ${
          resultObject.ADP.Yahoo
        } (${utilities.convertToPickPosition(resultObject.ADP.Yahoo)})`;
          ...
          const embedToSend = {
            embed: {
              ...
              author: {
                ...
                url: playerObject.url,
                icon_url:
                  "https://www.rotoworld.com/themes/custom/rotoworld_theme/favicon.ico",
              },
              fields: [
                {
                  name: "Latest News",
...
commands["!teamstats"] = (teamStatsObject) =>
  new Promise((resolve, reject) => {
    ...
    const valid = teamLookup.filter((obj) => {
      if (
        obj.abbreviation.toLowerCase() === input ||
        obj.mascot.toLowerCase() === input
      ) {
        return obj;
      }
    });
...
commands["!teamtargets"] = (object) =>
  new Promise((resolve, reject) => {
    ...
    const valid = teamLookup.filter((obj) => {
      if (
        obj.abbreviation.toLowerCase() === teamArg ||
        obj.mascot.toLowerCase() === teamArg
      ) {
        return obj;
      }
    });
...
commands["!help"] = () =>
  new Promise((resolve) => {
    resolve(
      "For command list, see https://github.com/chrisparsons83/RedditFFDiscordBot"
    );
  });
...
commands["!depthchart"] = (depthChartObject) => {
  ...
  return new Promise((resolve, reject) => {
    if (arg.length < 2) {
      ...
      let team = arg[0];
      let position = arg[1];
      const validPosition = ["qb", "rb", "wr", "te", "k"];
      ...
      const valid = teamLookup.filter((obj) => {
        if (
          obj.abbreviation.toLowerCase() === team ||
          obj.mascot.toLowerCase() === team
        ) {
          return obj;
        }
      });
  ...
  new Promise((resolve, reject) => {
    ...
    if (position === "dst") {
      ...
      players.forEach((item) => {
        ...
        if (team.length === 0) {
          reject(
            "Invalid team symbol/name. Please double check the team symbol/name."
          );
        } else {
          ...
          if (response.includes(",") || response.includes(" and ")) {
            message = `Boris says: **${response}** are in the same tier level at **${position.toUpperCase()}** with ${
              format === "full"
                ...
                ? "**0.5 PPR**"
                : "**Standard**"
            } format. Flip a coin or go with your gut.`;
          } else {
...
commands["!prediction"] = (discordObject) =>
  Promise.reject(
    "Stop trying to make !prediction happen. It's not going to happen."
  );
...
commands["!snaps"] = (discordObject) =>
  new Promise((resolve, reject) => {
    if (discordObject.args === undefined) {
      ...
      if (args.length < 3) {
        reject(
          "Missing an argument. Please check your query. `!snaps team, pos, week, year`"
        );
      }
      ...
      if (!teamArg || !weekArg || !posArg || !yearArg) {
        ...
        if (!yearArg) {
          reject(
            "Invalid year entered. No data available for the year selected."
          );
        }
      } else {
        snapcounts
          .getSnapcounts(teamArg, posArg, weekArg, yearArg)
          .then((array) => {
            const embed = {
              embed: {
                title: `${teamname} ${year}`,
                description: `${utilities.returnSnapcountStrings(
                  posArg,
                  weekArg,
                  array
                )}`,
            ...
            const string = utilities.returnSnapcountStrings(posArg, weekArg, array);
            resolve(embed);
          })
          .catch((err) => {
            reject(err);
...
commands["!drivestats"] = (discordObject) =>
new Promise((resolve, reject) => {
  const input = discordObject.args.toLowerCase();
  const valid = teamLookup.filter((obj) => {
    if (
      obj.abbreviation.toLowerCase() === input ||
      obj.mascot.toLowerCase() === input
  ...
  if (input === "") {
    ...
    const teamlogo = valid[0].logo;
    const symbol = valid[0].abbreviation;
    drivestats.getDriveStats(symbol)
    .then((object) => {
      if (Object.keys(object.team).length === 0 || Object.keys(object.league).length === 0) {
        reject(`No data available yet. Try again later.`);
      } else {
        const embed = {
          embed: {
            title: `${teamname}`,
            description: `${year} Offensive Drive Stats (League avg)`,
            color: 11913417,
            thumbnail: {
              url: teamlogo,
            },
            fields: [
              { name: "Number of offensive drives", value: `**${object.team.total_drive}** (${object.league.total_drive})` },
              { name: `Drive Success Rate (${object.team.drive_success_rate_rank})`, value: `**${object.team.drive_success_rate}%** (${object.league.drive_success_rate}%)` },
              {
                name: `Plays/dr (${object.team.plays_per_drive_rank})`,
                value: `**${object.team.plays_per_drive} plays** (${object.league.plays_per_drive})`,
                inline: true,
              },
              {
                name: `Yards/dr (${object.team.yards_per_drive_rank})`,
                value: `**${object.team.yards_per_drive} yds** (${object.league.yards_per_drive})`,
                inline: true,
              },
              {
                name: `Points/dr (${object.team.points_per_drive_rank})`,
                value: `**${object.team.points_per_drive} pts** (${object.league.points_per_drive})`,
                inline: true,
              },
              {
                name: `TD/dr (${object.team.td_per_drive_rank})`,
                value: `**${object.team.td_per_drive}%** (${object.league.td_per_drive}%)`,
                inline: true,
              },
              {
                name: `FG/dr (${object.team.fg_per_drive_rank})`,
                value: `**${object.team.fg_per_drive}%** (${object.league.fg_per_drive}%)`,
                inline: true,
              },
              {
                name: `TO/dr (${object.team.to_per_drive_rank})`,
                value: `**${object.team.to_per_drive}%** (${object.league.to_per_drive}%)`,
                inline: true,
              },
              {
                name: `INT/dr (${object.team.int_per_drive_rank})`,
                value: `**${object.team.int_per_drive}%** (${object.league.int_per_drive}%)`,
                inline: true,
              },
              {
                name: `Fumble/dr (${object.team.fumble_per_drive_rank})`,
                value: `**${object.team.fumble_per_drive}%** (${object.league.fumble_per_drive}%)`,
                inline: true,
              },
              {
                name: `Punts/dr (${object.team.punts_per_drive_rank})`,
                value: `**${object.team.punts_per_drive}%** (${object.league.punts_per_drive}%)`,
                inline: true,
              },
            ],
          },
