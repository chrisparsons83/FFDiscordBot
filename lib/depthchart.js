...
const getDepthChart = (depthChart, body) => {
  ...
  $('TR.row-dc-wht, TR.row-dc-grey').each(function eachRow() {
    let names = []
    let validPosition = ['wr', 'lwr', 'rwr', 'swr', 'rb', 'te', 'qb', 'k'];
    // get the text
    ...
    if (validPosition.includes(pos)) {
      ...
      if (pos === 'lwr' || pos === 'rwr' || pos === 'swr') {
       ...
       // special cases for 2 te/2 rb depth charts
        ...
        // special case where they dont list the wr position designation
        let key = pos + count;
        count ++;
        names.push(p1,p2,p3,p4,p5);
        names = names.map(x => {
        ...
        // check if wr1 exist in obj 
        if (key in rosterObj) {
          rosterObj[pos + count] = names;
        } else {
          rosterObj[key] = names;
        } 
      }
  ...
  } else if ('lwr' in rosterObj && 'rwr' in rosterObj) {
  ...
  //some team have no lwr, rwr or swr listed, only wrs with no designation
  if ('wr1' in rosterObj ) {
    let wr1 = rosterObj['wr1'], wr2 = rosterObj['wr2'], wr3 = rosterObj['wr3'];
    wr1.map((name, index) => {
      depthChart['wr'].push(name);
      depthChart['wr'].push(wr2[index]);
      depthChart['wr'].push(wr3[index]);
    });
    depthChart['wr'] = depthChart['wr'].filter(Boolean);
  } 
