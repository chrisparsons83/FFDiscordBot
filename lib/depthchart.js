...
exports.getRoster = (teamname) => {
    let city = teamLookup[teamname].name.split(' ')[0].toLowerCase();
    let team = teamLookup[teamname].name.split(' ')[1].toLowerCase();
    return new Promise((resolve, reject) => {
        let url = `https://www.cbssports.com/nfl/teams/depth-chart/${teamname}/${city}-${team}`;
            ...
            for (let i = 2; i < row1.length; i++) {
              ...
              if (pos) {
                for (let j = 0; j < players.length; j+=2) {
                  ...
                  let last = players[j+1].replace(/\s/g, '');
