...
exports.getPlayer = async (playerName) => {
  ...
  await page.goto(url);
  await page.waitFor('.player-highlight__player-first');
  ...
  const playerObject = await page.evaluate(() => {
    ...
    return {
      ...
      latestNews: document.querySelectorAll('.player-news-article__title')[0].innerText,
      latestImpact: document.querySelectorAll('.player-news-article__summary')[0].innerText,
