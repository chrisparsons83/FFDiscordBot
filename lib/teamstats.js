const teamStats = require('../stats/teamstats2017.json');
const leagueStats = require('../stats/leagueAverage2017.json');

const tools = {};

// return percentage
const getPercent = (num1, num2) => ((num1 / num2) * 100).toFixed(2);
...
const getTargets = (stat) => {
  ...
  const arr = [];
  let totalTargets = 0;
  Object.keys(stat.targets).forEach((element) => {
    arr.push([element, stat.targets[element].TOTAL]);
    totalTargets += stat.targets[element].TOTAL;
    return true;
  });
