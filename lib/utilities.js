  ...
  new Promise((resolve, reject) => {
    ...
    if (chooseListArray.length <= 1) {
      if (chooseList.indexOf(" ") !== -1) {
        ...
        reject(
          "Please give me at least two things to choose from - I like choices!"
        );
      }
    } else {
      resolve(
        chooseListArray[Math.floor(Math.random() * chooseListArray.length)]
      );
    }
  });
...
utilities.returnDepthChartStrings = (positionName, playerList, lastUpdated) => {
  ...
  if (positionName === "wr") {
    ...
    playerList.map((x) => {
      ...
      if (!x.includes("LWR") && !x.includes("RWR") && !x.includes("SWR")) {
        wrSorted.push(x);
      }
    });
...
utilities.rankPlayers = (object, players, position) => {
  ...
  object.string.forEach((item) => {
    ...
    if (position === "dst") {
      ...
      const tier = [];
      names.forEach((name) => {
        tier.push(utilities.findPlayer(name).no_suffix_name);
      });
      tiers.push(tier);
    }
  });
    ...
    ranking.forEach((playerObj) => {
      if (playerObj.score === ranking[0].score) {
        const name = playerObj.name
          ...
          .map(
            (strings) =>
              strings[0].toUpperCase() + strings.slice(1, strings.length)
          )
          .join(" ");
        names.push(name);
      }
...
utilities.returnSnapcountStrings = (position, week, array) => {
  let string = `${
    week !== "ALL" ? `Week ${week}` : "Total"
  } off snap percentages\n\n`;
...
utilities.argumentSplit = (string) => {
  let array = string.split(/[\s,]/).filter((n) => /\S/.test(n));
  return array;
};
...
utilities.removeDuplicatesInArray = (array) => {
