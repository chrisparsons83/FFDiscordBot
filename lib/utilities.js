...
utilities.findMatchInArrays= (item, array1, array2, array3) => {
  let match1 = array1.indexOf(item);
  let match2 = array2.indexOf(item);
  let match3 = array3.indexOf(item);
};

utilities.returnSnapcountStrings = (position, week, array) => {
  let string = `${(week !== 'ALL') ? `Week ${week}`: 'Total'} snap percentages\n\n`;
