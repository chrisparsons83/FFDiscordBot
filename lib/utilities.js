...
utilities.chooseOne = chooseList => new Promise(((resolve, reject) => {
  ...
  if (chooseListArray.length <= 1) {
    ...
    // if last item of chooseListArray is an empty string and list is <= 2, return the first item
    if (!chooseListArray[chooseListArray.length - 1]) {
      // pop empty string from the array or it will throw a cannot return an empty message error
      chooseListArray.pop();
    }
    console.log(chooseListArray)
...
utilities.returnDepthChartStrings = (teamname, positionName, roster) => {
