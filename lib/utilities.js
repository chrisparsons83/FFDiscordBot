...
utilities.chooseOne = chooseList => new Promise(((resolve, reject) => {
  ...
  if (chooseListArray.length <= 1) {
    ...
    let randomNumber;
    // if last item of chooseListArray is an empty string, return the first item
    if (!chooseListArray[chooseListArray.length - 1]) {
      randomNumber = 0;
    } else {
      randomNumber = Math.floor(Math.random() * chooseListArray.length);
    }
    resolve(chooseListArray[randomNumber]);
  }
}));
